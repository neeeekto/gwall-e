...
**Domain:** Go DDD/hexagonal microservices platform (Hardware-as-a-Service for data centers) + AI-agent conventions knowledge base
**Researched:** 2026-06-17
**Confidence:** HIGH (platform stack), MEDIUM (some convention/format choices are taste-driven)

> Scope reminder: the **first milestone** ships only the `memory-bank/`/`knowledge/` conventions and the tooling that enforces them (linters, formatters, commit hooks, rule-file layout). The platform-stack section below is the *target* the conventions should describe and standardize — it is not all built in milestone 1, but the rules must name these choices so every future service is wired the same way.
...
go get go.mongodb.org/mongo-driver/v2@latest          # NOTE: v2 path, replaces v1
...
# Test deps
go get github.com/onsi/ginkgo/v2@v2.23.4
go get github.com/onsi/gomega@v1.38.0
go get go.uber.org/mock@latest                          # gomock (optional)

# --- CLI / dev tooling (host install) ---
go install github.com/onsi/ginkgo/v2/ginkgo@v2.23.4
go install github.com/golangci/golangci-lint/v2/cmd/golangci-lint@v2.12.2
go install mvdan.cc/gofumpt@latest
brew install bufbuild/buf/buf                            # or: go install github.com/bufbuild/buf/cmd/buf@v1.71.0
brew install lefthook                                    # or: go install github.com/evilmartians/lefthook@latest

# --- commitlint (the only Node tooling; pin exact) ---
npm i -DE @commitlint/cli @commitlint/config-conventional
...
This is the actual deliverable of milestone 1. Findings on the 2025/26 standard:
...
**Recommended rule-file conventions to standardize in milestone 1:**
...
> Note: empirical evidence that these files measurably improve agent outcomes is still early/mixed. They are clearly worth doing for *consistency and reproducibility* (the stated milestone goal), but keep them lean and treat CI/linters as the real enforcement.

---

## Stack Patterns by Variant

**If a service must expose REST/browser clients (e.g. the React/Nx dashboard) later:**
...
**If manual DI in `main.go` becomes unwieldy as services multiply:**
...
**If domain events need cross-service propagation (future epics: autoheal, bulk ops):**
...
---
*Stack research for: Go DDD/hexagonal microservices + AI-agent conventions knowledge base*
*Researched: 2026-06-17*
